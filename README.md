...
4. If you do not already have the required packages (_discord.js_ and _axios_), please run the following command:
```bash
npm install
```
5. Create a file named .env in the main directory of the project.
6. Login to the [Discord Developer Portal]([https://discord.com/developers]), create a new application, and generate a bot token.
Paste the new bot token into your .env file like this:
```env
BOT_TOKEN="your_token_here"
```
7. Use the Discord Developer Portal installation tools to invite your new bot into your server.
8. Get a free API key from Weatherapi.com, and add it to your .env file like this:
```env
WEATHER_KEY="your_weather_key_here"
```
9. Start the bot by typing this into your terminal:
```bash
node index.js
```
